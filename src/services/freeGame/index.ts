...
export const handleFreeGameAnnouncement = async () => {
  ...
  const filteredGamesFromEpic = getFilteredFreeGames(
    formattedFreeGamesFromEpic,
    "currently_free_games"
  );
  ...
  const filteredGamesFromDB = getFilteredFreeGames(
    gamesFromDB,
    "currently_free_games"
  );
...
export const handleFreeGameCommand = async (
  categoryChoice: FreeGameCommandChoice,
...
const getIsAlreadyCalled = async () => {
  ...
  if (
    getIsDatesAreOnSameDay(
      new Date(db.data?.lastGamesUpdateTime),
      currentDate
    ) &&
...
const getIsSameGames = (firstGames: Game[], secondGames: Game[]) => {
  return firstGames.filter((firstGame) =>
    secondGames.some((secondGame) => firstGame.id === secondGame.id)
  );
};

const formatFreeGames = (gamesFromEpic: GameFromEpic[]) => {
  const formattedFreeGames = gamesFromEpic
...
const getFilteredFreeGames = (
  freeGames: Game[],
  category: FreeGameCommandChoice
) => {
