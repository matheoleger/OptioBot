    ...
    .setTitle(
      `**${name}** ${
        category == "currently_free_games"
          ? "est gratuit sur Epic"
          : "sera prochainement gratuit sur Epic"
      } !`
    )
    .setDescription(
      `Et oui, pas besoin de dépenser de l'argent pour jouer à **${name}** ! (clique sur le titre au dessus pour y accéder)`
    )
    .setColor(0x600273)
    .setImage(image)
    .setThumbnail(
